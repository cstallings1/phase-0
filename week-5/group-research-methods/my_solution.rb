...
def my_array_finding_method(source, thing_to_find)
  source.find_all {|i| i.to_s.include?(thing_to_find)}
end
...
#.find_all to select all items of an array that matched a given criteria
...
#.find_all will iterate through an array and find all elements that match the criteria. The elements that match are returned in an array.

#.include? is a great way to check if a string includes a given character(s). It is a string method so you would need to convert any Fixnums to a string using .to_s before applying .include?.
